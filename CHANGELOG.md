...
- **Monthly Performance Report Max now runs on Vendor Central accounts, and on a
  ...
  (repeat `--seller-id`, or `--brand <slug>` to take every live account from brand
  context), works out each account's channel from your MixShift account row, runs
  ...
  figures come from the daily campaign table on every channel. New flags:
  `--revenue-basis ordered|shipped` and `--attribution` for Vendor Central.
