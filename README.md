...
- **Explore your historical warehouse data.** Query, sample, and CSV-export the MixShift warehouse: Sponsored Ads (SP/SB/SD), DSP, Seller and Vendor Central revenue and orders, inventory, and catalog. Read-only. Large query and export results come back in full: the harness pages through big result sets automatically and streams them to a CSV file (pass `--out` to pick the path) instead of failing with a size error.
...
The plugin ships **26 skills**. Each is invoked naturally in chat: say what you want and Claude picks the right one. Two tiers, based on whether the skill needs a brand-context build first:
