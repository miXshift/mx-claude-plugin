...
```
plugins/mixshift-amazon-ppc/tmp/<brand-slug>-keyword-bid-health-<run_date>.data.json
```

If it is missing, run the pre-fetch script and wait for it to complete:
```bash
python3 plugins/mixshift-amazon-ppc/scripts/pre-fetch-data.py \
...
Run the renderer:
```bash
python3 plugins/mixshift-amazon-ppc/scripts/render-keyword-bid-health.py \
...
```
plugins/mixshift-amazon-ppc/tmp/<brand-slug>-keyword-bid-health-<run_date>.headline.json
```
