...
This section helps `search-term-negation` and `phrase-negative-discovery` calibrate what counts as relevant traffic.
...
This section feeds the Bottom Line of `daily-health-check`, `monthly-performance-report`, and any skill that interprets anomalies.
...
- **monthly-performance-report**: Lead the narrative with TACOS this quarter; ACOS is a derived metric for this brand and shouldn't be the headline.
- **search-term-negation**: Aggressive on competitor brand terms (we lose money on them); conservative on generic category terms.
- **portfolio-quick-scan**: Account is small enough that single-day spikes are noise; require 2+ day patterns before flagging YELLOW.
