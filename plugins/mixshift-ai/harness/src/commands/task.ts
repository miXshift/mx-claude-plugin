 ...
 *      the org store when it doesn't. A brand a teammate RETIRED stays READY
 *      with brand status 'retired' plus a warnings[] line (who, when, how to
 *      undo); it never blocks the run (brand retire, slice 1).
...
/**
 * 'retired' (brand retire): a teammate retired this brand for the team. It
 * is NOT a blocker: the run stays READY (one retired brand must never stop
 * the other brands of a multi-brand task) and a warnings[] line says who
 * retired it and how to undo. Stored task prompts generated by
 * mx-scheduled-task skip a brand whose status is 'retired' and say so at the
 * top of their output.
 */
type BrandStatus = 'present' | 'pulled' | 'unavailable' | 'retired';
...
interface BrandResult {
  slug: string;
  status: BrandStatus;
  /** status 'retired' only: who retired it and when (from the org manifest). */
  lifecycle?: {
    state: 'retired';
...
async function runPreflight(brandSlugs: string[], root: RootOptions): Promise<void> {
  ...
  try {
    ...
    for (const slug of brandSlugs) {
      ...
      if (retiredLc) {
        // Explicitly named brand: fetch its context if this sandbox has none
        // (explicit actions proceed), but a retired brand never BLOCKS the
        // run. The template skips it; an older stored prompt still reads it.
        if (!(await pathExists(ctxPath))) {
          const client = createContextSyncClient({ dataDirOverride: dataDir });
          await pull(slug, { client, dataDirOverride: dataDir, manifest: manifestBrands });
        }
        const by = retiredByClause(retiredLc);
        const w =
          `brand ${slug} is retired for your team (${by}). This task still names it: ` +
          'skip it in this run and say so at the top of the output, remove it from the task, ' +
          `or bring it back with \`${restoreCommand(slug)}\`.`;
        warnings.push(w);
        lines.push(statusLine('warn', `brand ${slug}: ${by}`));
        brands.push({
          slug,
          status: 'retired',
          lifecycle: {
        ...
        payload: {
          ...
          ...(brandSlugs.length > 0
            ? {
                ...
                brands_retired: brands.filter((b) => b.status === 'retired').length,
              }
            : {}),
        },
      },
