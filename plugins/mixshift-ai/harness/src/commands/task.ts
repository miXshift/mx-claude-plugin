 ...
 *      the org store when it doesn't. A brand a teammate RETIRED is handled
 *      exactly like any other brand (present / pulled / unavailable, and the
 *      run still includes it: retire never filters a report or a total), and
 *      additionally carries a `lifecycle` record plus one warnings[] line
 *      (who, when, how to stop including it, how to undo) (brand retire,
 *      slice 1).
...
interface BrandResult {
  ...
  status: BrandStatus;
  /**
   * Brand retire: present ONLY when a teammate retired this brand (from the
   * org manifest). Retire changes nothing else here: the brand keeps its
   * normal status and the run still includes it (the churn rule: retire
   * never filters a report or a total). Stored task prompts generated by
   * mx-scheduled-task put one line at the top of their output for it.
   */
  lifecycle?: {
    state: 'retired';
...
/** Budget for preflight's one manifest read (brand lifecycle). Fails open. */
const PREFLIGHT_MANIFEST_BUDGET_MS = 10_000;
...
async function runPreflight(brandSlugs: string[], root: RootOptions): Promise<void> {
  ...
  try {
    ...
    /** Brand retire, telemetry only: one state per --brand, in order. */
    const brandLifecycle: Array<'active' | 'retired' | 'unknown'> = [];

    // Brand retire: read the org manifest ONCE (lifecycle=all) so a brand a
    // teammate retired is reported as such even when its context.yaml is
    // already on disk. Fails open: an unreadable manifest (older service,
    // network) leaves every brand exactly as before, and the pulls below
    // fetch their own manifest as they always did.
    let manifestBrands: WireManifestBrand[] | undefined;
    if (brandSlugs.length > 0 && credentialSummary?.verified) {
      try {
        ...
        manifestBrands = undefined;
      }
    }

    for (const slug of brandSlugs) {
      const retiredLc = manifestBrands
        ? retiredLifecycleOf(findManifestBrand(manifestBrands, slug))
        : null;
      brandLifecycle.push(manifestBrands ? (retiredLc ? 'retired' : 'active') : 'unknown');
      const ctxPath = contextPath(slug, dataDir);

      // Brand retire: the run still includes a retired brand (retire never
      // filters a report or a total), so everything below treats it exactly
      // like an active one. It only gains a lifecycle record, one warn line
      // and one warnings[] entry saying who retired it and the two ways out.
      const lifecycle: Pick<BrandResult, 'lifecycle'> = retiredLc
        ? {
            lifecycle: {
              state: 'retired',
      ...
      if (retiredLc) {
        ...
        warnings.push(
          `brand ${slug} is retired for your team (${by}). This task still names it, so this ` +
            'run still includes it: say so in one line at the top of the output. To stop ' +
            'including it, remove it from the task; to bring it back for the team: ' +
            `\`${restoreCommand(slug)}\`.`,
        );
        ...
        payload: {
          ...
          ...(brandSlugs.length > 0
            ? {
                ...
                brands_retired: brands.filter((b) => b.lifecycle !== undefined).length,
              }
            : {}),
        },
      },
