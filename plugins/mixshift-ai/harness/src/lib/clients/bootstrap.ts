...
export interface BootstrapResult {
  ...
  /** True when an existing sub-brand `binding` block was carried forward
   *  verbatim across a `--force` re-bootstrap (FINDING 3, red team over
...
export async function bootstrapBrand(
  ...
  // 3. Preserve an existing sub-brand `binding` block across a --force
  //    re-bootstrap (FINDING 3, red team over PR #131). buildContext below
  //    has no knowledge of bindings — it rebuilds context.yaml purely from
  //    fresh warehouse data — so without this, `mixshift brand add <slug>
  //    --force` on an already-bound sub-brand would silently erase the
  //    binding AND auto-push the stripped context to the org store: the
  //    exact cross-brand smearing this design exists to prevent. Only
  //    consulted under --force (the non-force path already errors out at
  //    step 2 above when the directory exists, so there is nothing to read
  //    there — no-force behavior is unchanged).
  const existingBinding = options.force ? await readExistingBinding(ctxPath) : null;

  // 4. Build and validate the context.yaml shape
  const context = buildContext(suggestion, validAccounts, options.asOfDate) as Record<
    string,
    unknown
  >;
  if (existingBinding) {
  ...
  // 5. Write files (atomic per-file via tmp + rename)
...
/**
 * Read an existing context.yaml's `binding` block, tolerantly. A raw YAML
 * parse rather than a full `contextSchema` validation, so a binding block
 * survives even if some OTHER field on the existing file happens to be
 * invalid — the goal here is narrowly "don't lose the binding", not "the
 * whole existing file must be pristine". The extracted binding is then
 * checked against `bindingSchema` on its own, so a malformed EXISTING
 * binding is never carried forward as if it were valid.
 *
 * Returns null when there is no existing file (ENOENT — the normal
 * first-time bootstrap), the file is unreadable/unparseable, there is no
 * `binding` key, or the binding itself fails its own schema.
 */
async function readExistingBinding(ctxPath: string): Promise<BindingBlock | null> {
  let raw: string;
  try {
    raw = await readFile(ctxPath, 'utf-8');
  } catch {
    return null;
  }
  let parsed: unknown;
  ...
  const binding = (parsed as Record<string, unknown>).binding;
