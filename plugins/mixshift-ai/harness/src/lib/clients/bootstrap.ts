 ...
 *     intelligence) — that's the `mx-account-cold-start` skill's job,
...
*Run \`/mx-account-cold-start ${suggestion.slug}\` in Claude to complete AM intake (positioning, goals, structural events).*
...
**Next step:** run \`/mx-account-cold-start ${suggestion.slug}\` in Claude. The skill walks you through AM intake and fills in everything the bootstrap couldn't derive from the warehouse.
`;
}
