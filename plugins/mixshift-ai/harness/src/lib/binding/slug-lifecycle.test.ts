 ...
 * The manifest is the gateway's real response (wire fixture).
 */
...
) as Record<string, { body: { brands: WireManifestBrand[]; retired_count?: number } }>;
  ...
  it('asks for lifecycle=all and keeps the retired slug reserved', async () => {
    ...
    vi.mocked(createContextSyncClient).mockReturnValue({
      fetchManifest: async (opts?: FetchManifestOptions) => {
        seen.push(opts);
        const { brands, retired_count } = FIXTURE.manifest_all!.body;
        return { ok: true, brands, retired_count };
      },
    ...
    expect(existing.has('bravo-bottles')).toBe(true);
