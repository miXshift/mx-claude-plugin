...
When the user asks to retire, archive, or stop showing a brand their team no longer works on (a client left, a duplicate, a brand replaced by another), or to bring one back, **confirm with the user first, because it applies to their whole team**: *"Retiring Acme Snacks hides it from your team's shared brand context and bulk syncs for everyone on your team. Nothing is deleted, and it can be restored any time. Go ahead?"* Only after a yes, run `mixshift brand retire <brand-slug>` (add `--reason client_left|duplicate|superseded|other` and `--note "<short note>"` when the user gave one) or `mixshift brand restore <brand-slug>`, then relay the command's output as it is: it says what changed and what did not, who the change was recorded as, whether the local copy can be deleted, and the exact undo. Never retire a brand on your own initiative (a dormant or deactivated brand is still the user's call), never from a scheduled run, and never delete local files for the user. When a command mentions a brand is retired, relay who retired it and when, never call it deleted, and offer `mixshift brand restore <brand-slug>` if the user still works on it.
