...
description: "Set up Brand Context for a brand: build its Tier 3 brand context directory for a new Amazon PPC account from scratch. Collects account metadata, runs structured database queries, synthesizes into a typed context.yaml + narrative.md + corpora/, and renders a human-reviewable Brand Context page. Use this skill whenever the user mentions setting up or building brand context, onboarding a brand, a cold start, new account setup, account onboarding, or kicking off a new client — even if they don't say 'brand context' explicitly. ALSO the entry point for SUB-BRAND work on a seller account that holds many brands: use it when the user wants to find, list, or discover the sub-brands or brand labels inside one Amazon seller account, wants to build or review a promotion plan, wants to promote a label into a real brand, or wants to demote or reverse one — including phrasings like 'build a promotion plan', 'what brands are in this account', 'promote these labels', or 'set up each of our brands separately'. ALSO use it when the user asks to retire, archive, or restore a brand their team no longer works on (confirm first; it applies to the whole team). Always run before any other Amazon PPC skill on an account that doesn't yet have a populated ~/.mixshift/clients/[brand]/context.yaml."
---
...
**TACOS-primary target rule:** If `management.primary_metric: TACOS`, the human page must lead with the true TACOS goal (`management.tacos_goal_pct`; the deprecated `management.tacos_target_pct` alias still works on existing brands). `management.acos_target_pct` is only an ACOS-or-better proxy for bid math and must never be labeled as the TACOS target.
...
2. **Stockout candidates** (brain `stockouts`): contiguous FBA out-of-stock windows per ASIN (item, start and end dates, days in window). Present these qualitatively when confirming with the account manager; do not attach a dollar impact to a window (no reliable per-ASIN revenue attribution exists yet). **Limitation:** ASIN suppression-for-profitability events (Amazon de-ranks an ASIN despite inventory) are not detectable from inventory history; those still require AM input as `structural_events[]`.
...
- Keep runtime-only uploads separate. Forecast, H-Bridge, monthly-report screenshots are runtime inputs, not Brand Context gaps unless the operator says they do not exist.
...
- If forecast/H-Bridge/dimension bridge artifacts exist outside the DB, record them as runtime inputs required, not static context gaps.
...
**Standardized gap text for label_completeness (CS-24):** CS-24 returns per-campaign dimensions (`CampaignName`, `CampaignType`, spend, sales, ACOS) and does **not** assign an objective — you do, per brand.

Resolve each campaign's objective from **membership of a delimited name segment** in this brand's declared objective vocabulary (`campaign_structure.objectives`). Match on segment membership, never on segment position — accounts routinely carry the same token at different positions across their own campaigns. A campaign whose segments match nothing in the vocabulary is **unresolved**, and that is a valid, reportable outcome.

**Do not substitute `CampaignType` for an objective.** Campaign type is the ad product (Sponsored Products / Brands / Display), which is a different axis from why the campaign exists, and it already has its own dimension elsewhere in the product. An SB campaign can be brand defense or conquest; the format does not say which. Filling an objective from it would count every SB and SD campaign as "resolved" and inflate `coverage_spend_pct`, turning the one number meant to keep this honest into a confident-looking wrong answer. The retired hardcoded classifier made exactly this mistake: its `awareness_sb` and `awareness_sd` buckets meant "format known, intent unknown."

**Not available to this skill today:** the account's own per-campaign `campaign.Objective` label. Prefetch carries no per-campaign objective map, and the brain supplies only aggregates (`distinct_objectives`, `objective_tag_completeness_pct`). Do **not** run ad hoc SQL to obtain it — that violates the Hard Rule above. Where an account does tag its campaigns, those values reach you as vocabulary via `campaign_structure.objectives`, so segment matching already benefits from them; `coverage_spend_pct` will simply understate true coverage on a well-tagged account, which is the safe direction.
...
2. Sharing with your team happens automatically: the brand context publishes to your MixShift org store on every write (including this first setup), so teammates work from the same brand setup instead of rebuilding it, and the brand's structural events are recorded on the org brand timeline as declared stakes at the same time. A one-line notice confirms each share. `mixshift context push --brand <brand-slug>` remains available to publish on demand, and `mixshift timeline sync --brand <brand-slug> --dry-run` previews what will be recorded on the timeline.
...
## Retiring or restoring a brand (on request only)

When the user asks to retire, archive, or stop showing a brand their team no longer works on (a client left, a duplicate, a brand replaced by another), or to bring one back, **confirm with the user first, because it applies to their whole team**: *"Retiring Acme Snacks marks it retired for your whole team: it stops showing as active in your team's shared brand context, and bulk syncs skip it for teammates on the current MixShift plugin. Nothing is deleted, reports still include it, and it can be restored any time. Go ahead?"* Only after a yes, run `mixshift brand retire <brand-slug>` (add `--reason client_left|duplicate|superseded|other` and `--note "<short note>"` when the user gave one) or `mixshift brand restore <brand-slug>`, then relay the command's output as it is: it says what changed and what did not, who the change was recorded as, whether the local copy can be deleted, and the exact undo. The printed output is what counts: if it says the brand was retired or restored, the change is recorded, even if the command then exits with an error code (a known Windows issue after the output is printed), so do not run it again. Never retire a brand on your own initiative (a dormant or deactivated brand is still the user's call), never from a scheduled run, and never delete local files for the user. When a command mentions a brand is retired, relay who retired it and when, never call it deleted, and offer `mixshift brand restore <brand-slug>` if the user still works on it.
