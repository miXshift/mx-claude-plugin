...
The `confirmation.fields[]` array holds one entry per manifest field (find yours by `field.id`); read `effective_value` for the value this run will use. `scale_threshold_pct`, `pullback_threshold_pct` (bid math thresholds) and `acos_target` (reference ACoS, an optional override of the brand target) come back as fractions in [0,1] (e.g. `0.45` = 45%, not the whole number). Each is seeded from brand context where set, else absent.
...
- `pullback_threshold_pct`: if present, use it; else `acos_target × 1.5`; else (no target) `0.45` (45%). Label any default in the Bottom Line ("default, set to sharpen").
- `scale_threshold_pct`: if present, use it; else `acos_target × 0.7`; else `0.30` (30%). Label any default.
