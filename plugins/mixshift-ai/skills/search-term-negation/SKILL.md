...
```bash
mixshift telemetry emit skill.invoked --skill search-term-negation
# If natural-language trigger matched (NOT a /slash command), also run:
mixshift telemetry emit skill.trigger_phrase_matched --skill search-term-negation --trigger-phrase "<the user's exact phrase>"
```

At the END of this skill, run:

```bash
mixshift telemetry emit skill.completed --skill search-term-negation --outcome <ok|failed|deferred|skipped>
```
