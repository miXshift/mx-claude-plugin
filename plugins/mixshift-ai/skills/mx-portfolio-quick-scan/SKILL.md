...
**Brand context is optional — never fail closed on it (per account).** Render each account's card from whatever context is present (Tier-2 Brand Brain as fallback); cards sharpen as context accrues but never require full brand setup. The only hard requirement per account is `seller_id` + `account_type` (from `mixshift brand add`) — without those, that account's card shows "not bootstrapped — run `mixshift brand add <slug>`". When an account is missing targets/posture, render its card in observational mode and label it ("no ACoS target configured — set with `mixshift brand config <slug>`") instead of an error card. Other accounts continue regardless. Per account, load the brand-context fields in one call via `mixshift brand context resolve <slug> --json` — each carries `{value, source, fetched_at}` (`source: context` = ✓ confirmed, `brain` = ⊙ pre-filled; `null` = use the default).

---

## Data Source: per-account mx-daily-health-check sidecars

This skill is a **pure aggregator** of per-account `mx-daily-health-check` runs. It does not query the warehouse directly. The per-account skill (mx-daily-health-check) does the heavy lifting via `mixshift prefetch` + the skill model's classification; mx-portfolio-quick-scan reads the resulting sidecars and composes the cross-account view.

For each account in the portfolio config (today's data_date = T-1):

1. Look for the day's mx-daily-health-check sidecar at:
   ```
   ~/.mixshift/clients/<brand-slug>/runs/mx-daily-health-check/<data-date>-*.json
   ```
...
2. **If a sidecar for today is missing or older than 24 hours**, trigger the per-account skill via:
   ```bash
   mixshift prefetch --brand <brand-slug> --skill mx-daily-health-check --date <YYYY-MM-DD>
   ```
   Then run the mx-daily-health-check skill model on that account to compose its sidecar. Only after the per-account sidecar is on disk should you read it from this skill.
...
The per-account DHC sidecar carries everything mx-portfolio-quick-scan needs: `verdict`, `headline_metrics` (spend_t1, acos_t1, tacos_t30, pacing_*), `context_snapshot` (account_type, posture, primary_metric), and `structural_events_active`.
...
Portfolio-quick-scan emits no library-SQL itself. All warehouse access happens via per-account `mixshift prefetch --brand <slug> --skill mx-daily-health-check`. Each account's sidecar is the canonical input.

If you find yourself wanting to query the warehouse directly inside mx-portfolio-quick-scan, that signal probably belongs in the mx-daily-health-check skill instead — adding it there keeps the per-account drill-down and the portfolio view in lockstep.
...
// /tmp/pqs-sidecar-input.json
{
  ...
  "sql_calls": [
    {"id": "UPSTREAM:mx-daily-health-check",
     "params": {"sidecar_glob": "runs/*/mx-daily-health-check/YYYY-MM-DD-*.json",
...
```bash
mixshift telemetry emit skill.invoked --skill mx-portfolio-quick-scan
# If natural-language trigger matched (NOT a /slash command), also run:
mixshift telemetry emit skill.trigger_phrase_matched --skill mx-portfolio-quick-scan --trigger-phrase "<the user's exact phrase>"
```

At the END of this skill, run:

```bash
mixshift telemetry emit skill.completed --skill mx-portfolio-quick-scan --outcome <ok|failed|deferred|skipped>
```
