...
[ ] Prior-run sidecar loaded: ~/.mixshift/clients/<brand>/runs/mx-daily-health-check/<latest>.json
...
The `confirmation.fields[]` array holds one entry per manifest field (find yours by `field.id`); read `effective_value` for the value this run will use. `acos_target` and `tacos_target` (the targets DHC flags against) come back as fractions in [0,1] (e.g. `0.22` = 22%, not the whole number); `min_spend_to_flag` is int dollars, `hero_skus` is an ASIN list, `quiet_paused` is bool. Each is seeded from brand context where set, else absent (falls back to the documented default below).
...
- `acos_target`: a fraction in [0,1] (e.g. `0.22` = 22%). If absent, run observational (report ACoS as-is, do not flag vs target) per Step 0. Label any default in the report.
- `tacos_target`: a fraction in [0,1]. If absent, run observational (report TACoS as-is, do not flag vs target). Label any default.
...
```
~/.mixshift/clients/<brand-slug>/runs/mx-daily-health-check/<run_date>/data.md
```
...
After delivery, write a structured JSON sidecar capturing this run's inputs and headline outputs. Sidecars live at `~/.mixshift/clients/<brand-slug>/runs/mx-daily-health-check/<data-date>-<run-id>.json`. Schema source of truth: `plugins/mixshift-ai/shared/run-sidecar.schema.yaml`.
...
```bash
mixshift telemetry emit skill.invoked --skill mx-daily-health-check
# If natural-language trigger matched (NOT a /slash command), also run:
mixshift telemetry emit skill.trigger_phrase_matched --skill mx-daily-health-check --trigger-phrase "<the user's exact phrase>"
```

At the END of this skill, run:

```bash
mixshift telemetry emit skill.completed --skill mx-daily-health-check --outcome <ok|failed|deferred|skipped>
```
