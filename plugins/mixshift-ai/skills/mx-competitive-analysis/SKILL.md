...
```bash
mixshift telemetry emit skill.invoked --skill mx-competitive-analysis
# If natural-language trigger matched (NOT a /slash command), also run:
mixshift telemetry emit skill.trigger_phrase_matched --skill mx-competitive-analysis --trigger-phrase "<the user's exact phrase>"
```

At the END of this skill, run:

```bash
mixshift telemetry emit skill.completed --skill mx-competitive-analysis --outcome <ok|failed|deferred|skipped>
```
