...
- **All warehouse queries go through `mixshift prefetch`**, never inline SQL via the data-explore skill. Prefetch is what produces the audited artifacts and sidecar inputs.
...
[ ] Prior-run sidecar loaded from ~/.mixshift/clients/<brand>/runs/runaway-spend-check/
...
Fail closed: if any required field is missing or null, stop and direct the user to run the `account-cold-start` skill to complete intake.
...
```bash
mixshift prefetch --brand <brand-slug> --skill runaway-spend-check
```
...
The runner writes:
- `~/.mixshift/clients/<brand>/runs/runaway-spend-check/<date>/data.json` (machine-readable, all rows)
- `~/.mixshift/clients/<brand>/runs/runaway-spend-check/<date>/data.md` (capped markdown summary, ≤ 48 KB)
...
The sidecar lives at `~/.mixshift/clients/<brand>/runs/runaway-spend-check/<data-date>-<run-id>.json` for future drift comparisons (`mixshift sidecar compare`, coming later).
...
```bash
mixshift telemetry emit skill.invoked --skill runaway-spend-check
# If natural-language trigger matched (NOT a /slash command), also run:
mixshift telemetry emit skill.trigger_phrase_matched --skill runaway-spend-check --trigger-phrase "<the user's exact phrase>"
```

At the END of this skill, run:

```bash
mixshift telemetry emit skill.completed --skill runaway-spend-check --outcome <ok|failed|deferred|skipped>
```
