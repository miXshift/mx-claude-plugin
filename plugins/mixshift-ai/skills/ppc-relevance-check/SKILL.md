...
```bash
mixshift telemetry emit skill.invoked --skill ppc-relevance-check
# If natural-language trigger matched (NOT a /slash command), also run:
mixshift telemetry emit skill.trigger_phrase_matched --skill ppc-relevance-check --trigger-phrase "<the user's exact phrase>"
```

At the END of this skill, run:

```bash
mixshift telemetry emit skill.completed --skill ppc-relevance-check --outcome <ok|failed|deferred|skipped>
```
