...
```bash
mixshift telemetry emit skill.invoked --skill search-term-harvest
# If natural-language trigger matched (NOT a /slash command), also run:
mixshift telemetry emit skill.trigger_phrase_matched --skill search-term-harvest --trigger-phrase "<the user's exact phrase>"
```

At the END of this skill, run:

```bash
mixshift telemetry emit skill.completed --skill search-term-harvest --outcome <ok|failed|deferred|skipped>
```
