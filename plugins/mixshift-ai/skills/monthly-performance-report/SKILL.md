...
```bash
mixshift telemetry emit skill.invoked --skill monthly-performance-report
# If natural-language trigger matched (NOT a /slash command), also run:
mixshift telemetry emit skill.trigger_phrase_matched --skill monthly-performance-report --trigger-phrase "<the user's exact phrase>"
```

At the END of this skill, run:

```bash
mixshift telemetry emit skill.completed --skill monthly-performance-report --outcome <ok|failed|deferred|skipped>
```
