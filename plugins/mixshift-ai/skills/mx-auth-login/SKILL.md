...
(If `mixshift` isn't found, e.g. it isn't on PATH inside this sandbox, run the same command via `node "$MIXSHIFT_CLI" auth device-init ...`. If `$MIXSHIFT_CLI` is also empty (normal in Cowork, which does not run the session hook that sets it), resolve the CLI by scanning first: `MIXSHIFT_CLI="$(find / -maxdepth 9 -type f -path '*/harness/dist/cli.js' 2>/dev/null | head -1)"`, then use `node "$MIXSHIFT_CLI" ...`. Both being absent does not mean the plugin is missing; its CLI is in the plugin directory the scan finds.)
