...
# If $MIXSHIFT_CLI is already set, use it. Otherwise scan for the bundled CLI:
find / -maxdepth 9 -type f -path '*/harness/dist/cli.js' 2>/dev/null
# If that returns more than one path, take the highest version, not the first
# line. A machine keeps every version it has ever installed. Skip any path under
# a .trash folder, and read each remaining copy's version from
# .claude-plugin/plugin.json in its plugin folder (the path minus
# /harness/dist/cli.js), not from the path text and not by running it: many
# paths carry no version, and text order is not version order (as text, 0.8.10
# sorts before both 0.8.9 and 0.9.0). Set MIXSHIFT_CLI to the path you picked.
